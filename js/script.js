...
const renderOparation = (operation) => {
...
const updateBalans = () => {
	...
	.reduce((result, elemt)=> result + elemt.amount,0);
...
const deliteOperation = (event) => {
	...
	if(target.classList.contains('history_delete')){
		dbOperation = dbOperation
			.filter((operationId) => operationId.id !== target.dataset.id);
		init();
	}
...
const init = () => {
	...
	dbOperation.forEach((index) => {
		renderOparation(index);
	});
	updateBalans();
